...
class Discriminator(nn.Module):
    ...
    def forward(self, doc_seqs, summary_seqs):
        """
        Ref:
        ...
        Args:
            doc_seqs:     (batch_size, doc_max_seq_len)
            summary_seqs:  (batch_size, summary_max_seq_len)
        ...
        emb_q = self.embedding(doc_seqs).unsqueeze(1)  
        ...
        ## (batch_size, 1, max_seq_len, emb_dim)
        is_oov = (summary_seqs >= gen_opts.max_vocab_size).long()  #Mask indicating whether sampled word is OOV
        summary_seqs = (1 - is_oov) * summary_seqs.detach() + (is_oov) * UNK

        emb_r = self.embedding(summary_seqs).unsqueeze(1)
...
# discriminator = Discriminator(embedding = nn.Embedding(gen_opts.max_vocab_size, gen_opts.word_vec_size), dropout=dis_opts.dropout, fixed_embeddings=dis_opts.fixed_embeddings)
